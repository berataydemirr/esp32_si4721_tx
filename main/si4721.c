...
static void si_cmd(uint8_t *buf, size_t len)
{
    ...
    if (err != ESP_OK) ESP_LOGE(TAG, "I2C hatasi: %s", esp_err_to_name(err));
    i2c_cmd_link_delete(h);
}

static void si_prop(uint16_t prop, uint16_t value)
{
    uint8_t c[6] = { SI_CMD_SET_PROPERTY, 0x00, prop >> 8, prop & 0xFF, value >> 8, value & 0xFF };
    si_cmd(c, 6);
...
static void si_read(uint8_t *cmd, size_t cl, uint8_t *r, size_t rl)
{
    ...
    i2c_master_start(h);
    i2c_master_write_byte(h, (CFG_SI_ADDR << 1) | 1, true);
    for (size_t i = 0; i < rl - 1; i++) i2c_master_read_byte(h, &r[i], I2C_MASTER_ACK);
...
esp_err_t si4721_power_up(void)
{
    ...
    // ARG1=0x02: FUNC=TX, ARG2=0x0F: Dijital ses girisi (DCLK/DFS/DIN)
    uint8_t pu[3] = {SI_CMD_POWER_UP, 0x02, 0x0F};
    si_cmd(pu, 3);
    vTaskDelay(pdMS_TO_TICKS(500));
    if (!wait_cts()) {
        ESP_LOGE(TAG, "POWER_UP basarisiz!");
        return ESP_FAIL;
    }
...
esp_err_t si4721_configure(void)
{
    ...
    ESP_LOGI(TAG, "POWER: %d dBuV", CFG_TX_POWER);
...
void si4721_set_property(uint16_t prop, uint16_t value) { si_prop(prop, value); }
...
{
    si_prop(SI_PROP_TX_COMPONENT_ENABLE, CFG_COMPONENT | 0x0004);
...
{
    char p[8]; memset(p, ' ', 8);
    size_t len = strlen(text); if (len > 8) len = 8;
    memcpy(p, text, len);
    uint8_t c0[6] = {SI_CMD_TX_RDS_PS, 0x00, p[0], p[1], p[2], p[3]};
    si_cmd(c0, 6); vTaskDelay(pdMS_TO_TICKS(10));
    uint8_t c1[6] = {SI_CMD_TX_RDS_PS, 0x01, p[4], p[5], p[6], p[7]};
    si_cmd(c1, 6); vTaskDelay(pdMS_TO_TICKS(10));
    ESP_LOGI(TAG, "RDS PS: \"%.8s\"", p);
    return ESP_OK;
}

esp_err_t si4721_rds_set_rt(const char *text)
{
    size_t len = strlen(text); if (len > 64) len = 64;
    for (size_t i = 0; i < len; i += 4) {
        char b[4] = {' ',' ',' ',' '};
        for (size_t j = 0; j < 4 && (i+j) < len; j++) b[j] = text[i+j];
        uint8_t f = (i == 0) ? 0x07 : 0x03;
        uint8_t c[8] = {SI_CMD_TX_RDS_BUFF, f, 0x20, 0x00, (uint8_t)(i/4), 0x00, b[0], b[1]};
        si_cmd(c, 8); vTaskDelay(pdMS_TO_TICKS(5));
        uint8_t d[8] = {SI_CMD_TX_RDS_BUFF, 0x03, 0x20, 0x00, (uint8_t)(i/4), 0x00, b[2], b[3]};
        si_cmd(d, 8); vTaskDelay(pdMS_TO_TICKS(5));
    }
